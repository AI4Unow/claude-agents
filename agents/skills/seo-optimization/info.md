...
2. Download JSON → save as `$HOME/.claude/secrets/google_client_secret.json` (project) or `~/.claude/secrets/` (global)
3. Authenticate:
```bash
...
Config: `google_client_secret.json` in `$HOME/.claude/secrets/` (project or `~/.claude/secrets/` global)
