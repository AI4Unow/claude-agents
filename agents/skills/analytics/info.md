...
Credentials: `$HOME/.claude/secrets/ga_service_account.json` or `google_client_secret.json`

### Scripts
| Script | Purpose |
|--------|---------|
| `scripts/ga-config-loader.cjs` | Load credentials from $HOME/.claude/secrets |
...
```bash
# List accounts
node $HOME/.claude/skills/analytics/scripts/ga-list-accounts.cjs --summaries

# Run report
node $HOME/.claude/skills/analytics/scripts/ga-run-report.cjs \
